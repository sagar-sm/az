...
function generate() {
  ...
  if(characters.length == 0) {
    ...
    // var regex = new RegExp("^((?!.*?(" + OrString + ").*?).)*?$","gmi"); //Match all that don't contain character. Warning! Bad O(n).
    var regex = new RegExp("^.*(" + OrString + ").*$","mi"); // match all sentences that contain character
